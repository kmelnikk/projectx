...
begin
  ...
  begin
    // сохраняем значение имени из первого столбца
    // для сравнения с последующими строками

    n :=(SG.Cells[0,i]);

    // цикл по строкам таблицы, начиная со следующей после текущей
    for j := i+1 to SG.RowCount - 1 do
    begin
      // сравниваем имя в первом столбце текущей строки
      // с именем в первом столбце следующей строки
      if n = SG.Cells[0,j] then
      begin
        // складываем значения всех трех столбцов
        sum := sum + StrToInt(SG.Cells[0,j]) + StrToInt(SG.Cells[1,j]) + StrToInt(SG.Cells[2,j]);
      end;
    end;
  end;

  // выводим сумму на экран
  ShowMessage('ВинниПух:' + IntToStr(sum));
end;
